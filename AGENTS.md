...
- **同名重装载**：必须先 unmount（释放 serverName 预留）再 mount，顺序在 `reconcileContainer` 里已保证。连接死亡自愈按 **fiber 世代**：这一代曾经有工具、当前 `mcpToolCount===0`、未 `disabled` 才 unmount-then-mount；新一代首连失败交给官方内部重连。连续 3 个世代都是「有过工具再掉到 0」则 `give-up`；JSON `enabled:false` 不在装载集，不会被复活。`snapshot()` 只读内存，不驱动对账。只改 `tools.allow`/`deny` 就地换 `state.row` 再 `kickSweep`，不拆连接。
