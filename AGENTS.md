...
- `src/cli.ts`：`dsh-mcp` CLI（package.json `bin`）。`add`/`list`/`get`/`remove`/`status`，`--scope project|user|profile`、`--format yml|json`（env `DSH_MCP_CLI_FORMAT`，缺省 yml）、`--profile <name>`；`status` 只读各层文件与 `.mcp-diag.json` 摘要（不连接宿主）；`runCli(argv, io, deps)` 供进程内测试；手搓 argv 解析，不识别的 `-x` 视为服务器参数（positional），`--` 透传。
