...
- `src/cli.ts`：`dsh-mcp` CLI（package.json `bin`）。`add`/`list`/`get`/`remove`/`status`/`import`，`--scope project|user|profile`、`--format yml|json`（env `DSH_MCP_CLI_FORMAT`，缺省 yml）、`--profile <name>`；`status` 只读各层文件与 `.mcp-diag.json` 摘要（不连接宿主）；`import --from <file|->` 只接受 `{"mcpServers":{...}}`（同名默认跳过，`--overwrite` 覆盖，`--dry-run` 预演不写盘）；`runCli(argv, io, deps)` 供进程内测试；手搓 argv 解析，不识别的 `-x` 视为服务器参数（positional），`--` 透传。
...
- **配置指纹跳过重读**：`reconcileAll` 入口比对已知项目的 yml/json/cc 与用户层三路径的 `mtimeMs+size`；全同则跳过文件重读、复用上次期望集，仍跑健康巡检 / deny 重扫 / 摘要。
