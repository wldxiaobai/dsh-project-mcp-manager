...
**状态**：待裁决 ｜ **提出日期**：2026-09-12 ｜ **目标版本**：建议拆为 v0.4.4（非破坏）/ v0.5.0（次要）
...
| 编号 | 条目 | 组 | 优先级 | 破坏性 | 建议版本 |
|---|---|---|---|---|---|
| A0 | 传输值域镜像常量（C1/C2 前置） | 前置 | 高 | 否 | v0.4.4 |
| C1 | `sse` 条目的可执行报错 | 互通 | 高 | 否 | v0.4.4 |
| C2 | 接受 `httpUrl`；显式 `transport` 键；`url` 语义文档化 | 互通 | 高 | 否 | v0.4.4 |
| C4 | 「非本插件格式」诊断 + 共存声明 | 互通 | 高 | 否 | v0.4.4 |
| A3 | 诊断摘要段 + `dsh-mcp status` | 机制 | 高 | 否 | v0.4.4 |
| A5 | `dsh-mcp import`（批量导入 + dry-run） | 机制 | 高 | 否 | v0.4.4 |
| A1 | 连接健康巡检与自愈 | 机制 | 中高 | 否 | v0.5.0 |
| A2 | 停用/故障分类与外部改动兜底 | 机制 | 中 | 否 | v0.5.0 |
| A4 | 工具级 `allow`/`deny` | 机制 | 中高 | 否 | v0.5.0 |
| B2 | 工具预算护栏（只告警不裁剪） | 架构 | 中 | 否 | v0.5.0 |
| B3 | `ctx.provide` 查询服务面 | 架构 | 中 | 否 | v0.5.0 |
| B1 | 按需挂载（只挂活跃项目） | 架构 | 中（需裁决） | 语义变化 | v0.6.0 |
| C3 | VS Code `.vscode/mcp.json` 只读兼容层 | 互通 | 可选（需裁决） | 否 | v0.6.0 |
...
1. **A2b 兜底形态**：只做"触发点内指纹比对"（纯优化），还是增加默认关闭的低频 timer（能覆盖"完全无事件"）？
2. **A4 键名与语法**：`tools.allow`/`tools.deny`（DSH 方言）是否同时兼容读取生态键 `includeTools`/`excludeTools`？通配语法用 `*` 还是完整 glob？
3. **A5 作用域**：`dsh-mcp import` 是否允许 `--scope user`（写 `~/.dsh/mcp.yml` / `~/.dsh/mcp.json`）与 `--scope profile`？缺省目标是否与 `add` 保持一致？
4. **B1 卸载策略**：会话结束后立即卸载，还是保留宽限期（例如 N 分钟无会话）？
5. **B3 服务名与稳定性**：`projectMcp` 命名是否合适？是否需要对外承诺 API 稳定性（版本化）？
6. **C1 opt-in**：`type:"sse"` 是否提供一个显式开关（如条目级 `sseAsStreamableHttp: true`）尝试按 streamable-http 连？还是坚持只报错引导用户改配置？（政策依据见 §1.5；术语区分见 §1.6）
7. **C3 默认值与层序**：VS Code 层默认开还是关？层序放在 `cc-project` 之后是否合适？`envFile` 是否值得支持？
